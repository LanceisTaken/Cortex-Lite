...
**Rationale:** Anchors are curated ground truth for the most common tuples; the heuristic generalizes everywhere else. Keeping both paths and all adjustments deterministic preserves the "LLM never decides settings" guarantee and makes the engine testable with known-input to known-output assertions. Anchor lookup keys on the game's own Steam App ID or title, not the per-user `games.id`, because `setting_presets` is a shared catalog.
**Alternatives considered:** LLM-generated settings (rejected because it breaks the no-hallucination safety story). Mutating CPU-bound settings directly for bottlenecks (rejected as overreach given the current settings vocabulary; a surfaced `cpu_bottleneck` flag lets the explanation layer describe the tradeoff honestly without inventing setting changes).
...
**Date:** 2026-07-06
**Decision:** `ExplanationGenerator` caches Gemini prose in Redis keyed by deterministic recommendation inputs: forward mode uses `(game_id, gpu_tier, cpu_tier, ram_bucket, goal)`, and reverse mode uses `sha256(diff_structure, gpu_tier, cpu_tier, ram_bucket, goal)` with the diff's derived `label` excluded. Only successful Gemini responses are cached via explicit `get`/`put`; on any Gemini failure the deterministic static explanation is returned and not cached. The Gemini call stays synchronous with a 10-second timeout.
...
### Prod entrypoint fails fast on a bad SSM fetch instead of silently baking blank config
**Date:** 2026-07-09
**Decision:** `docker/app/entrypoint.prod.sh` captures `php artisan ssm:export` into a variable, aborts the boot (`exit 1`) if that command fails, and asserts `${APP_KEY:?...}` after `eval` — all *before* `php artisan config:cache`. The SSM client region is now resolved as `env('AWS_DEFAULT_REGION') ?: 'ap-southeast-1'` rather than via `env()`'s default argument.
**Rationale:** The original entrypoint ran `eval "$(php artisan ssm:export)"`. Under `set -e`, a failure *inside* a command substitution is not caught (its exit status is discarded), so a failed or empty fetch fell through and let `config:cache` bake a blank `APP_KEY` into `bootstrap/cache/config.php`. With `restart: unless-stopped`, that blank cached config then persisted indefinitely and survived restarts — a silent, hard-to-diagnose failure that presented as a 500 (`MissingAppKeyException`) on `/sanctum/csrf-cookie`. Failing fast turns a silent bad state into a loud crash-loop that surfaces in `docker compose logs`. The region `?:` change fixes a related trap: compose interpolating a blank host `AWS_DEFAULT_REGION` sets the container var to an empty string, which `env('AWS_DEFAULT_REGION', 'ap-southeast-1')` returns as `''` (present-but-empty shadows the default), leaving the SSM call with no region.
**Alternatives considered:** Leave the entrypoint lenient and rely on operators to notice blank config (rejected — this is exactly the failure that cost a debugging session; a boot that serves blank secrets should never be a *successful* boot). Add a healthcheck that greps `config('app.key')` (rejected — a healthcheck detects the bad state after the fact; refusing to cache blank config prevents it). Set `AWS_DEFAULT_REGION` only in the compose `environment:` block (kept as defense-in-depth via the host `.env`, but the `?:` coercion is the robust code-level guard).
**Consequences:** A misconfigured instance role, missing parameters, or wrong region now crash-loops the PHP services loudly instead of serving a blank-key app. The guard assumes `APP_KEY` is always among the fetched parameters (it must be). Operators must keep `/home/ec2-user/.env` populated with `AWS_DEFAULT_REGION` and `ECR_REGISTRY` so compose interpolation and image resolution work on recreate; these are non-secret and safe to persist on disk. Change takes effect on the next image rebuild/push.

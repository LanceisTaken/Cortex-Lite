...
> - **Steam connection UX rebuilt.** Steam OpenID login as primary; vanity URL resolution as fallback. Raw SteamID64 input removed entirely.
...
- [ ] **Implement vanity URL resolution as a manual fallback** (`POST /api/steam/connect-vanity`): user pastes their vanity URL or display name, backend calls `ResolveVanityURL`, persists the resolved SteamID64. This is the fallback for users who can't complete the OpenID redirect during the interview demo (it happens).
- [ ] Build a `SteamClient` service class (`app/Services/SteamClient.php`) wrapping the Steam API. Methods: `getOwnedGames($steamId)`, `getPlayerSummary($steamId)`, `resolveVanityURL($vanityName)`.
...
- [ ] PHPUnit tests for the Steam integration with HTTP fakes (Laravel's `Http::fake()`). Test cases: OpenID happy path, vanity URL resolution success/failure, private-profile rejection with structured error, transactional rollback on mid-sync failure.
